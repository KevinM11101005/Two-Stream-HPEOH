...
from common.utils.fitting import ScaleTranslationLoss, FittingMonitor
from common.utils.optimizers import optim_factory
...
class Model(nn.Module):
    def __init__(self, backbone, FIT, SET, regressor, final_layer, mlp, gau, cls_x, cls_y, decoder, kldloss):
    ...
    def forward(self, inputs, targets, mode):
        if cfg.att == 'cbam':
            b_feats, p_feats, s_feats = self.backbone(inputs['img']) # best, primary, secondary feats
        else:
            p_feats, s_feats = self.backbone(inputs['img'])
        
        if cfg.att == 'cbam':
            ps_feats = self.FIT(s_feats, p_feats)
            feats = self.FIT(ps_feats, b_feats)
        else:    
...
def get_model(mode):
    ...
    model = Model(backbone, FIT, SET, regressor, final_layer, mlp, gau, cls_x, cls_y, decoder, kldloss)
